# -*- coding: utf-8 -*-
"""
Created on Fri Sep  2 11:40:27 2016
...
# 新函数使用差值来计算极值（例如回撤超过某一固定数字）
def _identify_initial_pivot2(X, up_thresh, down_thresh):
    """Quickly identify the X[0] as a peak or valley."""
    x_0 = X[0]
    max_x = x_0
    max_t = 0
...
# 需要被调用的主函数
# 新函数使用差值来计算极值（例如回撤超过某一固定数字）
def peak_valley_pivots2(X, up_thresh, down_thresh):
    """
    Finds the peaks and valleys of a series.
...
def plot_pivots(X, pivots):
    ...
    pl.plot(np.arange(len(X)), X, '-', alpha=0.8)
